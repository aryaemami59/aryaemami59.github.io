...
createListFromArray(
  technicianGreenSmocks,
  $("#NeighborhoodTechnicianSmocks1")[0]
);
createListFromArray(
  technicianBlueSmocks,
  $("#SupercenterTechnicianSmocks1")[0]
);
...
function setList(array) {
  ...
  for (const i of array) {
    ...
    const descOI = Array.from(
      itemContainers.querySelectorAll("p.descVendor#descOI")
    );
    const descGNFR = Array.from(
      itemContainers.querySelectorAll("p.descVendor#descGNFR")
    );
    const descSOC = Array.from(
      itemContainers.querySelectorAll("p.descVendor#descSOC")
    );
    const descVS = Array.from(
      itemContainers.querySelectorAll("p.descVendor#descVS")
    );
    const descMS = Array.from(
      itemContainers.querySelectorAll("p.descVendor#descMS")
    );
    const descCOV = Array.from(
      itemContainers.querySelectorAll("p.descVendor#descCOV")
    );
    ...
    //   if (e.key == 'ArrowDown') {
    //     e.preventDefault();
    //     console.log(list.querySelector('li:first-of-type'));
    //     list.querySelector('li:first-of-type').focus();
    //   }
    // })
    // console.log(list[1]);
...
search.addEventListener("input", (event) => {
  ...
  if (value && value.trim().toLowerCase().length > 0) {
    ...
    //     if (e.key == 'ArrowDown') {
    //       e.preventDefault();
    //       list.querySelector(`li:nth-of-type(${number})`).focus();
    //       console.log(list.querySelector(`li:nth-of-type(${number})`));
    //       number ++;
    //     }
    //   })
    // }
...
// button1.textContent = "GNFR";
// // const divider = document.createElement("span");
// // divider["id"] = "divider";
// button1.classList.add('badge');
