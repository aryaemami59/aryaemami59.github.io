const vials = [
	{
		...
		keywords: ["amber", "ten", "vials", "09670503346", "mck", "mckesson", "orderinsite", "oi"],
		McKesson: true,
		OrderInsite: true,
	},
	{
	...
	},
	{
		...
		keywords: ["cap", "10", "13", "blue", "mck", "mckesson", "orderinsite", "oi"],
		McKesson: true,
		OrderInsite: true,
	},
	{
		...
		keywords: ["cap", "30", "40", "60", "blue", "mck", "mckesson", "orderinsite", "oi"],
		McKesson: true,
		OrderInsite: true,
	},
	{
		...
		keywords: ["mck", "mckesson", "orderinsite", "oi", "ounce", "berry", "oval"],
		McKesson: true,
		OrderInsite: true,
	},
	{
	...
	},
	{
		...
		keywords: ["mck", "mckesson", "orderinsite", "oi", "imz", "needle", "gauge"],
		McKesson: true,
		OrderInsite: true,
	},
	{
	...
	},
	{
		...
		keywords: ["mck", "mckesson", "orderinsite", "oi", "mask", "face mask"],
		McKesson: true,
		OrderInsite: true,
	},
	{
	...
	{
		name: 'ReliOn Insulin Syringes 6mm(15/64") 1 mL(Green Stripes)',
	...
	{
		name: 'ReliOn Insulin Syringes 6mm(15/64") 1/2 mL(Blue strips)',
	...
	{
		name: 'ReliOn Insulin Syringes 6mm(15/64") 3/10 mL(Purple Strips)',
	...
	{
		name: 'ReliOn Insulin Syringes 8mm(5/16") 1 mL(Green Stripes)',
	...
	{
		name: 'ReliOn Insulin Syringes 8mm(5/16") 1/2 mL(Blue Stripes)',
	...
	{
		name: 'ReliOn Insulin Syringes 8mm(5/16") 3/10 mL(Purple Stripes)',
	...
	},
	{
		...
		// itemNumber: "PHARMACY ORAL SYRINGE 1ML",
		// itemNumber: "100616526",
		keywords: ["gnfr"],
		McKesson: false,
		OrderInsite: false,
		GNFR: true,
	...
	{
		name: "FY22 Diabetes Reli On Mod Update",
	...
	{
		name: "McKesson Prevent Lancets 21G x 1.8mm",
		itemNumber: "1016300",
...
const MCKItemsArr = vials.filter(e => e.McKesson);
const OIItemsArr = vials.filter(e => e.OrderInsite);
const GNFRItemsArr = vials.filter(e => e.GNFR);
const SOCItemsArr = vials.filter(e => e.signOrderCatalog);
const VSItemsArr = vials.filter(e => e.vaxServe);
const MSItemsArr = vials.filter(e => e.medSurge);
const COVItemsArr = vials.filter(e => e.covap);
const FORSItemsArr = vials.filter(e => e.FORS);
const OIExclusiveItemsArr = vials.filter(e => !e.McKesson && e.OrderInsite);
...
const [classMCK, classOI, classGNFR, classSOC, classVS, classMS, classCOV, classFORS] = vendorClass;
...
function bigCollapseHideFunc(bigCollapseHideButtonGNFR) {
	$(bigCollapseHideButtonGNFR).on("click", function (e) {
		e.preventDefault();
		const $button = $(this);
		const apple = $(this).parent().find("li").not(".collapseParent");
		$(apple).each(function () {
...
function clickVendor(
	...
	if (liOI1.length < 10 && !target.hasAttribute(addedtooi)) {
		...
		const alternateDesc = Array.from(itemContainers.querySelectorAll(descVendor));
...
// let arrOIItemNames = [];
// let GNFRItemsArr = [];
// let arrGNFRItemNames = [];
// let SOCItemsArr = [];
// let arrSignOrderCatalogItemNames = [];
// let VSItemsArr = [];
// let arrVaxServeItemNames = [];
// let MSItemsArr = [];
// let arrMedSurgeItemNames = [];
// let COVItemsArr = [];
// let arrCovapItemNames = [];
// let FORSItemsArr = [];
// let arrFORSItemNames = [];
// these are the term specific arrays that are used in the searchFunc and createListFromArray.
let items = [];
let items1 = [];
let addedItems = [];
let addedItemsMCK = [];
...
// this function checks the vendor and assigns that vendor as a property to that items.
...
// this function looks at the main vials array and if the name of
// the items matches the name of an item in the main array,
// it returns whatever property is asked for in the argument.
...
checkVendor("vaxServe");
checkVendor("medSurge");
checkVendor("covap");
checkVendor("FORS");
...
function keywordSearch(itemToFind, arr) {
	...
	vials.forEach(element => {
		const x = Array.from(element.keywords);
		x.forEach(e => {
			if (e.match(g)) {
				arr.push(element);
			}
...
// this functions removes the duplicate items from an array and returns a new array as the result.
...
searchFunc("neighborhood.*technician coats", technicianGreenSmocks);
searchFunc("supercenter.*technician coats", technicianBlueSmocks);
...
createListFromArray(MCKItemsArr, $("#McKessonItems1")[0]);
createListFromArray(OIItemsArr, $("#OrderInsiteItems1")[0]);
createListFromArray(GNFRItemsArr, $("#GNFRItems1")[0]);
createListFromArray(amberVials, $("#Vials1")[0]);
createListFromArray(caps, $("#Caps1")[0]);
createListFromArray(SOCItemsArr, $("#SignOrderCatalog1")[0]);
...
createListFromArray(technicianGreenSmocks, $("#NeighborhoodTechnicianSmocks1")[0]);
createListFromArray(technicianBlueSmocks, $("#SupercenterTechnicianSmocks1")[0]);
...
createListFromArray(liquidBottles1arr, $("#LiquidBottles1")[0]);
createListFromArray(FORSItemsArr, $("#Equipment1")[0]);
createListFromArray(COVItemsArr, $("#Covap1")[0]);
createListFromArray(MSItemsArr, $("#CardioChekandA1C1")[0]);
createListFromArray(PPE1arr, $("#PPE1")[0]);
createListFromArray(VSItemsArr, $("#ImmunizationSupplies1")[0]);
...
function removeButtonFunc(removeButtonMCK, array, image, char, redMCK, addedItemsMCK, addedtoMCK) {
	removeButtonMCK.addEventListener("click", function d(e) {
		...
		const QR = e.target.parentNode.parentNode.querySelector(".QRContainer").firstChild;
		...
		li.forEach(element => {
			if (desc == element.childNodes[0].textContent) {
				$(element).css("text-decoration", "none");
			}
		});
		const resultsList = Array.from(list2.querySelectorAll("li"));
		...
		const alternativeDesc = Array.from(itemContainers.querySelectorAll("p.descVendor"));
		...
		if (column !== null) {
			...
			for (let i = 0; i < liVendor.length; i++) {
				...
				for (let u = 0; u < li1.length; u++) {
					const li1Loop = li1[u];
					const li1Text = li1Loop.childNodes[0].textContent;
					if (li1Text == descVendor) {
						$(li1Loop).show();
						$(li1Loop).removeAttr(addedtomck);
					}
				}
				for (let k = 0; k < listUl.length; k++) {
					const listUlLoop = listUl[k];
					const navLi = listUlLoop.querySelectorAll("li");
					for (let q = 0; q < navLi.length; q++) {
						const navLiLoop = navLi[q];
						if (descVendor == navLiLoop.textContent) {
							$(navLiLoop).css("text-decoration", "none");
						}
					}
				}
			...
			array.length = 0;
			// addedItems.length = 0;
			orderNumberVendor.textContent = 0;
		}
	});
}
bigClearFunc(bigClearButtonMCK, arr, ".redMCK", "redMCK", "data-addedtomck");
bigClearFunc(bigClearButtonOI, arr1, ".redOI", "redOI", "data-addedtooi");
bigClearFunc(bigClearButtonGNFR, arr2, ".redGNFR", "redGNFR", "data-addedtognfr");
bigClearFunc(bigClearButtonSOC, arr3, ".redSOC", "redSOC", "data-addedtosoc");
bigClearFunc(bigClearButtonVS, arr4, ".redVS", "redVS", "data-addedtovs");
bigClearFunc(bigClearButtonMS, arr5, ".redMS", "redMS", "data-addedtoms");
bigClearFunc(bigClearButtonCOV, arr6, ".redCOV", "redCOV", "data-addedtocov");
bigClearFunc(bigClearButtonFORS, arr7, ".redFORS", "redFORS", "data-addedtofors");

function removeDuplicatesFunc(removeDuplicates, addedtomck, array, image, char, descMCK) {
...
function onClick(li) {
	for (i = 0; i < vials.length; i++) {
		if (li.childNodes[0].textContent == vials[i].name) {
			let index = vials.map(e => e.name).indexOf(li.childNodes[0].textContent);
			const target = li;
			const itemNumber1 = vials[index].itemNumber;
			...
			smallLinkOI.setAttribute("href", "https://acloud.orderinsite.com/rs/logon/walmart");
			...
			smallLinkFORS.setAttribute("title", "This Link Will Take You To The FORS Website");
			...
			copyItemNumber(itemNumberOI, itemNumber1);
			copyItemNumber(itemNumberGNFR, itemNumber1);
			copyItemNumber(itemNumberSOC, itemNumber1);
			copyItemNumber(itemNumberVS, itemNumber1);
			copyItemNumber(itemNumberMS, itemNumber1);
			copyItemNumber(itemNumberCOV, itemNumber1);
			copyItemNumber(itemNumberFORS, itemNumber1);
			copyItemNumber(descMCK, nameMain);
			copyItemNumber(descOI, nameMain);
			copyItemNumber(descGNFR, nameMain);
			copyItemNumber(descSOC, nameMain);
			copyItemNumber(descVS, nameMain);
			...
			collapseFunc(collapseButtonOI);
			collapseFunc(collapseButtonGNFR);
			collapseFunc(collapseButtonSOC);
			collapseFunc(collapseButtonVS);
			collapseFunc(collapseButtonMS);
			collapseFunc(collapseButtonCOV);
			collapseFunc(collapseButtonFORS);
			const liMCK1 = Array.from(MckessonItems.querySelectorAll(".liVendor.MCK"));
			const liOI1 = Array.from(OrderInsiteItems.querySelectorAll(".liVendor.OI"));
			const liGNFR1 = Array.from(GNFRItems.querySelectorAll(".liVendor.GNFR"));
			const liSOC1 = Array.from(signOrderCatalogItems.querySelectorAll(".liVendor.SOC"));
			...
			bigCollapseHideFunc(bigCollapseHideButtonSOC);
			bigCollapseHideFunc(bigCollapseHideButtonVS);
			bigCollapseHideFunc(bigCollapseHideButtonMS);
			bigCollapseHideFunc(bigCollapseHideButtonCOV);
			bigCollapseHideFunc(bigCollapseHideButtonFORS);
			bigCollapseShowFunc(bigCollapseShowButtonMCK);
			bigCollapseShowFunc(bigCollapseShowButtonOI);
			bigCollapseShowFunc(bigCollapseShowButtonGNFR);
			bigCollapseShowFunc(bigCollapseShowButtonSOC);
			bigCollapseShowFunc(bigCollapseShowButtonVS);
			...
			//     addedItemsMCK.push(li.childNodes[0].textContent);
			...
			//     orderNumberFunc(orderNumberOI);
			//     const alternateDesc = Array.from(
			//       itemContainers.querySelectorAll("p.descVendor:not(#descOI)")
			//     );
			//     alternateDesc.forEach((e) => {
			//       if (descOI.textContent === e.textContent) {
			//         $(e).addClass("duplicate");
...
function buttonClick() {
	...
	for (let i = 0; i < li.length; i++) {
		...
		avoidDuplicatesOnEnter(listNew);
...
function setList(array) {
	...
	for (const i of array) {
		...
		avoidDuplicatesOnClick(li);
		avoidDuplicatesOnEnter(li);
		const descMCK = Array.from(itemContainers.querySelectorAll("p.descVendor#descMCK"));
		const descOI = Array.from(itemContainers.querySelectorAll("p.descVendor#descOI"));
		const descGNFR = Array.from(itemContainers.querySelectorAll("p.descVendor#descGNFR"));
		const descSOC = Array.from(itemContainers.querySelectorAll("p.descVendor#descSOC"));
		const descVS = Array.from(itemContainers.querySelectorAll("p.descVendor#descVS"));
		const descMS = Array.from(itemContainers.querySelectorAll("p.descVendor#descMS"));
		const descCOV = Array.from(itemContainers.querySelectorAll("p.descVendor#descCOV"));
		const descFORS = Array.from(itemContainers.querySelectorAll("p.descVendor#descFORS"));
		function add(desc, addedtoMCK) {
			desc.forEach(e => {
				if (li.childNodes[0].textContent === e.textContent) {
...
// this function clears the search results field anytime the search button is pressed to avoid duplicate items in the search results.
function combine() {
	if (box.value !== "" || list.firstChild) {
		replace(list2);
		items.length = 0;
		buttonClick();
		replace(list);
...
// this function clears the input field results everytime the input value changes to avoid duplicate and inaccurate search results.
function clearList() {
	while (list.firstChild) {
		list.removeChild(list.firstChild);
	}
}
// this function removes the first child while the item has a first child, it's used to clear list items everytime new results are added.
function clean(mck) {
	while (mck.firstChild) {
		mck.removeChild(mck.firstChild);
	}
}
// this function returns no results when no results are found.
...
search.addEventListener("input", event => {
	...
	if (value && value.trim().toLowerCase().length > 0) {
		...
		const liMCK2 = Array.from(list.querySelectorAll("li#li[data-MCK*=true"));
		const liOI2 = Array.from(list.querySelectorAll("li#li[data-OI*=true"));
		const liGNFR2 = Array.from(list.querySelectorAll("li#li[data-GNFR*=true"));
		const liSOC2 = Array.from(list.querySelectorAll("li#li[data-SOC*=true"));
		const liVS2 = Array.from(list.querySelectorAll("li#li[data-VS*=true"));
		const liMS2 = Array.from(list.querySelectorAll("li#li[data-MS*=true"));
		const liCOV2 = Array.from(list.querySelectorAll("li#li[data-COV*=true"));
		const liFORS2 = Array.from(list.querySelectorAll("li#li[data-FORS*=true"));
		function addLogo(liVendor2, vendorName) {
			liVendor2.forEach(e => {
		...
		addLogo(liMCK2, "McKesson");
		addLogo(liOI2, "OrderInsite");
		addLogo(liGNFR2, "GNFR");
		addLogo(liSOC2, "Sign Order Catalog");
		addLogo(liVS2, "VaxServe");
		addLogo(liMS2, "MedSurge");
		addLogo(liCOV2, "Covap");
		addLogo(liFORS2, "FORS");
	} else {
		clearList();
	}
});
